//cunado comience el juego busca una casilla aletoria 

function SearchMove(){
	///*/*/*/*/***RPIORIDADES*/*/*/*/*///
	//contar numero fichas maquina
	//si es superior a 1
		//formar linea con fichas maquina
		//cortar linea rival
	...
	machine = CheckTurn_Count(1);
	if (machine > 1){
		Completed = false; //SI LA LINEA NO ESTA COMLETA

		//formar linea maquina 	COMPRELTADA=FALSO	REVISAR SI SE CUMPLE COMPLETARROW
		...
		//si no se hizo nada de lo anterior
			if (machine == 3){//EDITAR LA CANTIDAD DE FICHAS
				CellFind = false;
		...
		if (Find_final == true){
			...
			if (CheckTurn_Count(turn_value) == 3){
				...
				if (turn_value == 1){
					...
					//la borramos
					Machine_Sellected_x = Find_Lost_x;
					Machine_Sellected_y = Find_Lost_y;
...
function CompleteDiag(x, turn_value){
	...
	if (CheckDiag(x, turn_value) == 2){
		...
		//si esta vacia
		if (Find_final == true){
			//si hay tres fichas****/*/*/*/*/*/*//REVISON 3 FICJAS/*/*/*/
			if (CheckTurn_Count(turn_value) == 3){
				//preguntar de quien queremos completar la linea
				//si es la maquina -> armar linea
				if (turn_value == 1){
